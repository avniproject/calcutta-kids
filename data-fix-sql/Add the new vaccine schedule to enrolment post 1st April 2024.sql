...
INSERT INTO checklist_item(completion_date, checklist_id, uuid, version , organisation_id, observations, checklist_item_detail_id, last_modified_by_id, last_modified_date_time, created_by_id , created_date_time)
SELECT
	NULL,
	checklist.id,
	uuid_generate_v4(),
...
JOIN checklist ON
	checklist.program_enrolment_id = enl.id AND
    	checklist.is_voided = FALSE 
WHERE
	enl.id NOT IN (
		SELECT
			enl.id
		...
		JOIN checklist_item ON
			...
			AND checklist_item.is_voided = FALSE
