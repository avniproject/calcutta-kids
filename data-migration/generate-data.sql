...
FROM mother m
  INNER JOIN woman_registration wr ON wr.entity_id != '' AND wr.entity_id :: INT = m.id) TO '/tmp/migrations/MotherAndPregnancyRegistration.csv' (format CSV);
...
inner join mother on pregnancy_registration.entity_id != '' AND pregnancy_registration.entity_id :: INT = mother.id) TO '/tmp/migrations/PregnancyEnrolment.csv' (format CSV);

-- Mother Enrolment [csv created]
COPY(select
  m2.uuid as individual_uuid,
  reg.enrolment_uuid
from mother_enrolment reg join mother m2 on reg.mother_id = m2.id) TO '/tmp/migrations/MotherEnrolment.csv' (format CSV);
...
FROM child c
  ...
  INNER JOIN woman_registration wr ON wr.entity_id != '' AND wr.entity_id :: INT = m.id
  LEFT OUTER JOIN child_status status on status.child_id = c.id) TO '/tmp/migrations/ChildRegistrationAndEnrolment.csv' (format CSV);
...
from anc_first_trimester tr
 ...
 inner join pregnancy_registration reg on reg.entity_id != '' AND reg.entity_id :: INT = tr.entity_id::int
   and tr.date::DATE BETWEEN lastmenstrualperiod::DATE AND (lastmenstrualperiod::DATE + '4 month'::INTERVAL)) TO '/tmp/migrations/PregnancyANC1.csv' (format CSV);
...
-- 3 & 7 are rough numbers to group encounters of an enrolment, if there are multiple pregnancy for an individual
 inner join mother m on m.id = tr.entity_id :: int) TO '/tmp/migrations/PregnancyANC2.csv' (format CSV);
...
-- 6 & 11 are rough numbers to group encounters of an enrolment, if there are multiple pregnancy for an individual
 inner join mother m on m.id = tr.entity_id :: int) TO '/tmp/migrations/PregnancyANC3.csv' (format CSV);
...
order by mother.uuid, reg.enrolment_uuid) TO '/tmp/migrations/PregnancyLabTests.csv' (format CSV);
...
order by woman_lab_test_form.id) TO '/tmp/migrations/MotherLabTests.csv' (format CSV);
...
from child_doctor_visit
  ...
  left outer JOIN child_registration cr ON child.id = cr.entity_id :: INT) TO '/tmp/migrations/ChildDoctorVisitFile1.csv' (format CSV);;
...
from child_doctor_visit_and_follow_up
  ...
  left outer join followupform_child_doctor_visit_and_follow_up on child_doctor_visit_and_follow_up.id = followupform_child_doctor_visit_and_follow_up.parent_id) TO '/tmp/migrations/ChildDoctorVisitFollowup.csv' (format CSV);;
...
from woman_doctor_visit_and_follow_up
  ...
  left outer join followupform_woman_doctor_visit_and_follow_up on woman_doctor_visit_and_follow_up.id = followupform_woman_doctor_visit_and_follow_up.parent_id) TO '/tmp/migrations/MotherDoctorVisitFollowup.csv' (format CSV);;
...
from delivery_details_and_pnc1 pnc
  ...
  left join pregnancy_registration reg on reg.pregnancydetailid :: INT = pd.id)TO '/tmp/migrations/PregnancyDeliveryAndPNC1.csv' (format CSV);
...
from pnc2
  ...
  join child c on c.id = reg.entity_id :: int)TO '/tmp/migrations/ChildPNC.csv' (format CSV);
...
from child_death_form
 ...
 left join child_registration reg on reg.entity_id::int = c.id)TO '/tmp/migrations/ChildExit_Reason-Death.csv' (format CSV);
...
from child_gmp gmp
  ...
  left join child_registration reg on reg.entity_id::int = c2.id)TO '/tmp/migrations/ChildGMP.csv' (format CSV);
...
left outer join mother_enrolment enrolment on m2.id = enrolment.mother_id)TO '/tmp/migrations/SES.csv' (format CSV);
...
COPY (select m.uuid as individual_uuid,
 ...
 left outer join mother_enrolment enrolment on m.id = enrolment.mother_id)TO '/tmp/migrations/MotherGMP.csv' (format CSV);
...
COPY (select c.uuid as individual_uuid,
 ...
 left join child_registration on child_registration.entity_id::int = c.id)TO '/tmp/migrations/ChildHomeVisit.csv' (format CSV);
...
t.given_elsewhere, t.immunisation_date taken_date, t.due_date, t.comments
 ...
 left join child_registration on child_registration.entity_id::int = c2.id)TO '/tmp/migrations/ChildImmunisationTaken.csv' (format CSV);
...
from child_lab_test_form a
  ...
  left join child_registration reg on e.id = nullif(reg.entity_id, '') :: int)TO '/tmp/migrations/ChildLabTests1.csv' (format CSV);
...
where beneficiarytype = 'child')TO '/tmp/migrations/ChildLabTests2.csv' (format CSV);
...
where beneficiarytype = 'mother')TO '/tmp/migrations/MotherLabTests1.csv' (format CSV);
...
where beneficiarytype = 'pregnantWoman')TO '/tmp/migrations/PregnancyLabTests1.csv' (format CSV);
...
from doctor_visit dv
  ...
  left join mother_enrolment reg on e.id = reg.mother_id
where beneficiary = 'mother')TO '/tmp/migrations/MotherMedicationsAndTestsInDoctorVisit.csv' (format CSV);
...
from doctor_visit dv
  ...
  left join child_registration reg on e.id = nullif(reg.entity_id, '')::int
where beneficiary = 'child')TO '/tmp/migrations/ChildMedicationsAndTestsInDoctorVisit.csv' (format CSV);
...
from doctor_visit dv
  ...
  left join mother e on e.id = nullif(dv.entity_id,'')::int
  left join pregnancy_registration reg on e.id = nullif(reg.entity_id, '')::int
where beneficiary = 'pregnantWoman')TO '/tmp/migrations/PregnancyMedicationsAndTestsInDoctorVisit.csv' (format CSV);
...
from woman_doctor_visit dv
  ...
  left join mother e on e.id = nullif(dv.entity_id,'')::int
  left join mother_enrolment reg on e.id = reg.mother_id
where beneficiary = 'mother')TO '/tmp/migrations/MotherMedicationsAndTestsInDoctorVisit2.csv' (format CSV);
...
from woman_doctor_visit dv
  ...
  left join mother e on e.id = nullif(dv.entity_id, '')::int
  left join pregnancy_registration reg on e.id = nullif(reg.entity_id, '')::int
where beneficiary = 'pregnantWoman')TO '/tmp/migrations/PregnancyMedicationsAndTestsInDoctorVisit2.csv' (format CSV);
...
from child_doctor_visit dv
  ...
  left join child e on e.id = nullif(dv.entity_id, '')::int
  left join child_registration reg on e.id = nullif(reg.entity_id, '')::int
where beneficiary = 'child')TO '/tmp/migrations/ChildMedicationsAndTestsInDoctorVisit2.csv' (format CSV);
...
where beneficiarytype = 'pregnantWoman')TO '/tmp/migrations/PregnancyFollowup.csv' (format CSV);
...
where beneficiarytype = 'mother')TO '/tmp/migrations/MotherFollowup.csv' (format CSV);
...
inner join child_registration reg on nullif(reg.entity_id,'')::int = c.id)TO '/tmp/migrations/ChildFollowup.csv' (format CSV);
...
where beneficiarytype = 'mother')TO '/tmp/migrations/MotherFollowup2.csv' (format CSV);
...
where beneficiarytype = 'pregnantWoman')TO '/tmp/migrations/PregnancyFollowup2.csv' (format CSV);
...
where beneficiarytype = 'child')TO '/tmp/migrations/ChildFollowup2.csv' (format CSV);
